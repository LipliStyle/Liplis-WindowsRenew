...
namespace Liplis.Xml
{
    public class XmlLinq : XmlMost
    {
        ...
        protected override void readXml()
        {
            ...
            catch (WebException ex)
            {
                ...
                if (ex.Status == System.Net.WebExceptionStatus.ProtocolError)
                {
                    ...
                    //応答ステータスコードを表示する
                    LpsLogController.writingLog(this.GetType().Name, MethodBase.GetCurrentMethod().Name, "Web例外 " + errres.StatusCode + ":" + errres.StatusDescription + Environment.NewLine + "接続先はココ→" + xmlFilePath);
                ...
                {
                    //XML例外 XMLが読み込めませんでした。
                    LpsLogController.writingLog(this.GetType().Name, MethodBase.GetCurrentMethod().Name, "Web例外 XMLが読み込めませんでした。サーバーが存在しない可能性があります。" + Environment.NewLine + "接続先はココ→" + xmlFilePath);
                    responseCode = 11;
                }
            }
            catch (System.Exception)
            {
                ...
                LpsLogController.writingLog(this.GetType().Name, MethodBase.GetCurrentMethod().Name, "XMLその他例外ファイルにアクセスできません。ファイルが開かれている可能性があります。" + Environment.NewLine + xmlFilePath);
                responseCode = 12;
            }
        }
        ...
        protected void readXmlNonConvert(string url)
        {
            xmlFilePath = url;
            readXmlNonConvert();
        }
