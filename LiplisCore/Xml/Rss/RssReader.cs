...
namespace Liplis.Xml.Rss
{
    public class RssReader : XmlLinq, IDisposable
    {
        ...
        public int interpretationRssType()
        {
            try
            {
                try
                {
                    ...
                    {
                        LpsLogController.writingLog(this.GetType().Name, MethodBase.GetCurrentMethod().Name, "interpretationRssType XMLが不正: " + this.rssUri);
                    }
                }
                catch { return def; }

                if (xmlDoc == null) { return def; }
                if (xmlDoc.Root == null) { return def; }
        ...
        /// </summary>
        #region getMoveInfo
        public bool getMoveInfo()
        {
            foreach (string title in urlTitleList)
            {
                if (title.Contains("移転"))
                {
                    return true;
                }
            }

            foreach (string descriptiont in discriptionList)
            {
                if (title.Contains("移転"))
                {
                    return true;
